...
async function loadBrowserSolvePow() {
  const browserModule = await import(pathToFileURL(path.join(__dirname, "..", "browser", "cap_wasm.js")).href);
...
function assertCases(solvePow, cases, label) {
  for (const [salt, target, expectedNonce] of cases) {
    const actualNonce = solvePow(salt, target).toString();
    assert.equal(
      actualNonce,
      String(expectedNonce),
      `${label} mismatch for salt=${JSON.stringify(salt)} target=${JSON.stringify(target)}`,
    );
  }
}
...
async function main() {
  ...
  assertCases(nodeSolvePow, regressionCases, "node wrapper");
  assertCases(browserSolvePow, browserCases, "browser wrapper");

  console.log(`verified ${regressionCases.length} node cases and ${browserCases.length} browser cases`);
}
