...
运行服务端时，可以通过设置 `CORS_ORIGIN` 环境变量来更改兑换和生成质询时的默认 CORS 设置。默认值为 `*`，即允许所有来源。可以用逗号分隔添加多个来源，例如 `domain1.tld,domain2.tld,...`。

## 静态资源服务器

静态资源服务器默认关闭。将 `ENABLE_ASSETS_SERVER` 环境变量设置为 `true` 即可开启，资源会从 `/assets` 端点提供。

然后，请确保将 `WIDGET_VERSION` 和 `WASM_VERSION` 设置为你想要提供的验证组件和 WASM 文件的正确版本。默认值为 `latest`，会提供最新版本的组件和 WASM 文件，但不建议在生产环境使用，因为可能引入破坏性变更。
...
质询端点按客户端 IP 使用固定窗口进行速率限制，默认每 5 秒 30 次请求。你可以在控制台的 **Settings** 中修改全局限制（或通过 `PUT /settings/ratelimit`），也可以在某个站点密钥的 **Configuration** 标签页中单独覆盖。超出限制时，请求会收到 `429` 响应，并带有 `X-RateLimit-Remaining: 0` 响应头。

`/siteverify` 端点面向服务器间调用，因此默认不做速率限制。

### 代理后的客户端 IP

Standalone 通过依次检查 `X-Forwarded-For`、`X-Real-IP` 和 `CF-Connecting-IP` 头来识别客户端，都没有时回退到套接字地址。如果你的反向代理使用其他头，请在环境变量中设置 `RATELIMIT_IP_HEADER`（或在控制台 **Settings > Headers** 中设置 IP 头）。例如在 Cloudflare 后面，可以设置为 `cf-connecting-ip`。
...
如果多个 Cap 部署（或其他应用）共用同一个 Redis 实例，请设置 `REDIS_PREFIX` 为所有键添加命名空间。例如 `REDIS_PREFIX=cap:` 会把会话存为 `cap:session:...`、指标存为 `cap:metrics:...` 等。默认值为空，因此现有部署不受影响。
...
Standalone 支持 [RSW 时间锁谜题](../rsw.md)，作为可选启用的、抗 GPU 的 SHA-256 PoW 替代方案。它按站点密钥配置，因此可以让部分密钥使用 RSW，其余密钥继续使用默认的 SHA-256 质询。
...
难度由 **RSW squarings** 滑块（即 `t` 参数——客户端必须计算的连续平方次数）控制。默认值为 `75_000`，在现代硬件上大约相当于 300-800ms 的客户端计算量。调低可以让质询更轻量，调高则限流更强。有效范围为 `10_000`-`300_000`。
...
Cap Standalone 支持 JavaScript instrumentation（浏览器环境检测）质询来对抗工作量证明求解器，并提供阻止无头浏览器求解的选项。新建站点密钥时，instrumentation 质询默认开启。
