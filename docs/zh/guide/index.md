...
Cap 是一款自托管的 CAPTCHA（人机验证），它用不可见的工作量证明取代了图像谜题。用户只需点击一个复选框，计算会在浏览器中静默完成，他们的任何数据都不会离开你的服务器。没有 Google，没有遥测，没有按请求计费。

Cap 由两部分组成：负责运行质询并显示复选框的**验证组件**，以及负责签发质询、校验解答的**服务端**。大约五分钟就能把两者都跑起来。
...
::: tip
查看[最新版本](https://github.com/tiagozip/cap/releases)以确定要固定的版本号。在高安全性场景下，你也可以自托管这个文件，而不是从 CDN 加载。
:::
...
- `<your-instance>` 是你 Cap 服务端的公开 URL，例如 `cap.example.com`。它必须能被访客访问到，所以不能是 `localhost`。
- `<site-key>` 是控制台中的站点密钥。

提交时，你的服务器会连同其他字段一起收到 `cap-token`。跳到[第 3 步](#_3-验证令牌)进行验证。
...
- `<key_secret>` 是控制台中的**秘密密钥**，不是控制台的 `ADMIN_KEY`。把这两者搞混是最常见的配置错误。
...
3. 再次发送同一个令牌，这次应该失败——这说明一次性机制在正常工作。
